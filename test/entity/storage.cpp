...
TEST_CASE("storage") {
    ...
    SECTION("misc") {
        ...
        auto& v2 = storage.emplace(Entity(2), Vector2{2.0, 4.0});
        REQUIRE(v2.x == 2.0);
        REQUIRE(v2.y == 4.0);
        REQUIRE(storage.size() == 2);
        ...
        auto& v3 = storage.emplace(Entity(3), Vector2{3.0, 5.0});
        ...
        REQUIRE_FALSE(storage.contain(Entity(2)));
        REQUIRE(**storage.find(Entity(3)) == v3);
        REQUIRE(**storage.find(Entity(0)) == v);
        storage.remove(Entity(0));
        REQUIRE_FALSE(storage.contain(Entity(0)));
        REQUIRE(**storage.find(Entity(3)) == v3);
