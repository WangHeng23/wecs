...
public:
    ...
    const_iterator begin() const { return std::as_const(*this).begin(); }
...
private:
    internal::CompressPair<packed_container_type, Hasher> packed_;
    ...
    inline size_t index(const Key& key) const {
        return fast_mod<size_t>(Hasher{}(key), sparse_.size());
    }
};
