...
#include "wecs/entity/sigh_mixin.hpp"
#include "wecs/entity/storage.hpp"
#include "wecs/entity/system_constructor.hpp"
...
struct storage_for<basic_sparse_set<EntityT, PageSize>, Type> {
    using type = sigh_mixin<basic_storage<EntityT, Type, PageSize, std::allocator<Type>, config::type_info>>;
};
...
public:
    ...
    WECS_ANY get_mut(EntityT entity,
    ...
    storage_for_t<Type>& assure_storage() noexcept {
        ...
        if (pools_[idx] == nullptr) {
            pools_[idx] =
                std::make_unique<storage_for_t<Type>>(WECS_GET_TYPE_INFO(Type));
            type_infos_[idx].type_info = WECS_GET_TYPE_INFO(Type);
    ...
    auto& regist_enter_system_to_state(T value) noexcept {
        if (auto it =
            ...
            WECS_ASSERT(false, "state not exists");
        }
        return *this;
    }
    ...
    auto on_update() noexcept {
        if constexpr (std::is_same_v<T, entity_type>) {
            WECS_ASSERT(false, "entity don't has update listener");
            return null_entity;
        } else {
...
private:
    ...
    struct TypeInfo final {
        ...
        WECS_ANY (*convert_to_any)(self_type&, entity_type);

        template <typename T>
        static WECS_ANY convert_type_to_any(self_type& reg,
                                            entity_type entity) {
            auto ref = WECS_MAKE_ANY_REF(reg.get_mut<T>(entity));
            return ref;
        }
    };
