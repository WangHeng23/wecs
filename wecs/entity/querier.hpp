...
template <typename... Ts>
struct get_condition_inner_types<wecs::without<Ts...>> {
    using type = type_list<Ts...>;
};

template <typename... Ts>
struct get_condition_inner_types<wecs::only<Ts...>> {
    using type = type_list<Ts...>;
};
...
template <typename T, typename... Ts, typename EntityT, typename PoolContainer,
          typename F>
bool check_condition(type_list<T, Ts...>, EntityT entity,
...
struct check_without_satisfied_one {
    ...
    bool operator()(EntityT entity, const PoolContainer& pools) const {
        ...
        return idx >= pools.size() || !pools[idx] ||
               !pools[idx]->contain(entity);
    }
};
...
bool check_only_satisfied(type_list<Ts...>, EntityT entity,
...
bool check_condition(EntityT entity, const PoolContainer& pools) {
    if constexpr (Condition::is_only) {
        return check_only_satisfied(typename Condition::require_list{}, entity,
                                    pools);
    } else {
...
public:
    ...
    using query_types = typename internal::get_query_condition_t<query_condition>::require_list;
    ...
    using pool_container = typelist_to_tuple_t<list_foreach_t<query_types, storage_for_with_constness>>;
    using pool_container_reference = pool_container&;
    using iterator = internal::querier_iterator<
        EntityT,
        std::decay_t<decltype(std::declval<typename RegistryT::pool_base_type>().packed())>,
        pool_container>;
    using const_iterator = const iterator;
    using entity_container = typename RegistryT::pool_base_type::packed_container_type;
    using entity_type = EntityT;

    basic_querier(pool_container pools, const entity_container& entities) noexcept
