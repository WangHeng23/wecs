...
public:
    ...
    constexpr sparse_set_iterator(const container_type* container, difference_type offset)
...
public:
    ...
    entity_type insert(entity_type entity) noexcept {
        ...
        WECS_ASSERT(traits::entity_mask != internal::entity_id(entity), "invalid entity id");
        ...
        return internal::construct_entity<entity_type>(
            0, static_cast<typename traits::entity_mask_type>(packed_.size() - 1u));
    }
    ...
    entity_numeric_type& pump(entity_type src, entity_type dst) noexcept {
        WECS_ASSERT(!empty(), "sparse set must not empty when pump element");
    ...
    size_t index(entity_type entity) const noexcept {
        ...
        WECS_ASSERT(page < sparse_.size(), "entity must exists in sparse set when call index");
    ...
    const_iterator begin() const noexcept {
        return internal::sparse_set_iterator<type>{
            &packed_,
            static_cast<typename internal::sparse_set_iterator<type>::difference_type>(packed_.size())};
    }
