...
public:
    ...
    constexpr storage_iterator(const container_type* container, difference_type offset)
 ...
 * @tparam PageSize  the page size for basic_sparse_set
 * @tparam Allocator the allocator to allocate Payload
...
public:
    ...
    Payload& insert(entity_type entity, Args&&... args) noexcept {
        WECS_ASSERT(!base_type::contain(entity), "storage already has entity");
    ...
    void remove(entity_type entity) noexcept override {
        WECS_ASSERT(base_type::contain(entity), "entity not exists");
        auto idx = this->index(entity);
    ...
    basic_storage(const config::type_info& type_info = {})
...
private:
    ...
    TypeInfo type_info_ = {};  // for dynamic reflection runtime type info
    ...
    template <typename Compare>
    void do_sort(int left, int right, Compare cmp) {
        ...
        while (low < high) {
            while (low < high && cmp(*pivot, *payloads_[high])) {
                high--;
            }
            payloads_[low] = payloads_[high];
            base_type::packed()[low] = base_type::packed()[high];
            while (low < high && cmp(*payloads_[low], *pivot)) {
                low++;
            }
        ...
        do_sort(left, low - 1, cmp);
        do_sort(low + 1, right, cmp);
    }
};
