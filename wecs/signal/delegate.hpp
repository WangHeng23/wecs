...
public:
    ...
    void connect(delegate_pointer_type fn) {
        payload_ = nullptr;
        fn_ = fn;
    }

    template <typename Payload>
    void connect(delegate_pointer_type fn, Payload& payload) {
        payload_ = &payload;
        fn_ = fn;
    }
        ...
        if constexpr (std::is_invocable_r_v<
            ...
            return [](const void*, Args... args) -> Ret {
                ...
                return static_cast<Ret>(std::invoke(
                    Func, 
                    std::forward<list_element_t<args_list, Index>>(std::get<Index>(forward_args))...));
            };
        } else {
            WECS_ASSERT(false, "unsupport bind function!");
            return nullptr;
        }
    }
    ...
    delegate_pointer_type wrap(Payload& payload, std::index_sequence<Index...>) {
        ...
        if constexpr (std::is_invocable_r_v<
            ...
            return [](const void* instance, Args... args) -> Ret {
                ...
                return static_cast<Ret>(std::invoke(
                    Func,
                    *static_cast<Payload*>(const_cast<constness_as_t<void, Payload>*>(instance)),
                    std::forward<list_element_t<args_list, Index>>(std::get<Index>(forward_args))...));
            };
        } else {
            WECS_ASSERT(false, "unsupport bind function!");
